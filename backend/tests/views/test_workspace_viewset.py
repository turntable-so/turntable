...
class WorkspaceViewSetTestCase(TestCase):
    ...
    def test_update_workspace(self):
        ...
        response = self.client.put(f'/workspaces/{self.workspace1.id}/', data, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['name'], 'Updated Workspace')

    def test_retrieve_workspace(self):
        self.user.current_workspace = lambda: self.workspace1  # Mock current_workspace method
