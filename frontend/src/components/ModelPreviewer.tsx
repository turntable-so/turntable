...
function ColumnPreviewer({ column }: { column: any }) {
  const columnDescription =
    column.description || column.ai_description || "No description";
  const isUsingAiDescription = !column.description && column.ai_description;
    ...
    <>
      <div className="p-4">
        <div className="flex flex-col space-y-2">
          ...
          <div className="font-mono font-medium text-gray-400 flex items-center text-xs">
            <ColumnTypeIcon dataType={column.type} className="size-4 mr-1" />
            {column.type}
          </div>
...
export default function ModelPreviewer({
  ...
  const assetDescription =
    asset.description || asset.ai_description || "No description";
  const isUsingAiDescription = !asset.description && asset.ai_description;
      ...
      <div className="flex justify-between items-center text-center bg-muted border-y">
        {context === "NOTEBOOK" && (
          <div className="flex justify-between w-full">
            ...
            <Button
              onClick={insertAssetSqlIntoCurrentContext}
              className="gap-1 flex items-center"
              size="sm"
              variant="ghost"
            >
              ...
              <div>
                ...
                <div className="space-y-2">
                  {asset.columns.map((column: any, i: number) => (
                    <ColumnPreviewer key={column.id} column={column} />
                  ))}
                </div>
              </div>
            )}
