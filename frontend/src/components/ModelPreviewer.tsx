...
export default function ModelPreviewer({ asset, context, clearAsset }: { context?: 'NOTEBOOK' | 'LINEAGE', asset: any, clearAsset: () => void }) {
    ...
    const description = asset.description?.trim() === '' ? undefined : asset.description
    const aiDescription = asset.ai_description?.trim() === '' ? undefined : asset.ai_description
                            ...
                            <div className='border-none p-4 space-y-4'>
                                ...
                                <div>
                                    ...
                                    description ? (
                                        ...
                                        <div className='font-medium text-gray-600 gap-1 space-x-1'>
                                          <span>{aiDescription}</span>
                                          <TooltipProvider>
                                            <Tooltip>
                                              <TooltipTrigger>
                                                <Badge variant='outline' className="px-1 font-light">
                                                  <WandSparkles className="w-4 h-4" />
                                                  </Badge>
                                              </TooltipTrigger>
                                              <TooltipContent className="w-64">
                                                <p>This description was generated by AI.</p>
                                              </TooltipContent>
                                            </Tooltip>
                                          </TooltipProvider>
                                          </div>
                            ...
                            <div>
                                ...
                                <div className='space-y-2'>
                                    {asset.columns.map((column: any, i: number) => {
                                      const columnDescription  = column.description?.trim().length > 0 ? column?.description : null;
                                      const aiColumnDescription = column.ai_description?.trim().length > 0 ? column?.ai_description : null;
                                        ...
                                        <Fragment key={i}>
                                          <div className='p-4'>
                                            <div className=''>
                                              ...
                                              {
                                                  columnDescription ?
                                                <div className='pt-1 font-normal'>{columnDescription}</div>
                                                : aiColumnDescription ?
                                                <div className='pt-1 font-normal gap-1 space-x-1'>
                                                  <span>{aiColumnDescription}</span>
                                                  <TooltipProvider>
                                                    <Tooltip>
                                                      <TooltipTrigger>
                                                        <Badge variant='outline' className="px-1 font-light">
