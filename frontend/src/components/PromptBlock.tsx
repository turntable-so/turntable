...
export default function PromptBlock() {
    ...
    const [content, setContent] = useState([{ type: 'text', text: 'Initial text ' }]);
    const editableRef = useRef(null);

    useEffect(() => {
        // Convert content state to HTML
        const html = content.map(part => {
    ...
    const fetchRecommendations = () => {
        ...
        const relevantSources = sources
            .map((source: any) => ({ ...source, relevancy: (2 * source.table.toLowerCase().includes(searchTerm) ? 1 : 0) + (source.id.toLowerCase().includes(searchTerm) ? 1 : 0) }))
                ...
                <Card className='rounded-md border-none'>
                    <CardContent className='p-2 text-muted-foreground bg-muted'>
                        {recommendations.map((recommendation, i) => (
                            // @ts-ignore
                            <div className='cursor-pointer py-0.5 px-2 hover:bg-gray-200 rounded-md' onClick={() => selectRecommendation(i)} key={i}>{recommendation.table} ({recommendation.database})</div>
                        ))}
                    </CardContent>
                </Card>
            )}
            <Card className='rounded-md'>
                <CardContent className='p-4 text-muted-foreground bg-muted/50'>
                    <textarea
                        ...
                        className='resize-none w-full bg-transparent outline-none text-md' /> </CardContent>
            </Card>
