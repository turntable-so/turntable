...
export default function Notebook({ notebook }: { notebook: Notebook }) {
  ...
  return (
    <div style={{
      ...
      ref={divRef} className="p-2 mt-12 px-8 ">
      <div style={{
        maxWidth: `${maxWidth}px`,
      }} className=''>
        <input
          onChange={(e) => setActiveNotebook({ ...activeNotebook, title: e.target.value })}
        ...
        <div className='mt-12'>
          ...
          <div className='space-y-16'>
            {activeNotebook.blocks.map((block: Block, i: number) => {
              if (block.type === 'query') {
                return (
                  <QueryBlock
                    ...
                    // @ts-ignore
                    title={block.title} key={block.id} onSqlChange={(sql: string) => onSqlChange(block.id, sql)} sql={block.sql} records={block.records} />
                )
              }
              ...
              return (
                <div key={i}>Unknown block type</div>
              )
            })}
          </div>
        </div>
        <div className='mt-16'>
          <div className='flex items-center space-x-4 opacity-60 hover:opacity-100'>
            <Separator />
            <Button onClick={addPromptBlock} variant='secondary'><Plus className='w-4 h-4 text-gray-400' /></Button>
            <Separator />
          </div>
        </div>
        <div className='h-24' />
