                ...
                <CardContent className='text-xl font-medium my-8 space-y-4'>
                    {loading ? (
                        ...
                        workspaces.map((workspace: Workspace) => (
                            <div className=" flex justify-between  items-center hover:bg-gray-50 p-4 rounded-md cursor-pointer" key={workspace.id} onClick={() => handleSwitchWorkspace(workspace.id)}>
                                <div className="flex items-center space-x-4">
                                    <WorkspaceIcon name={workspace.name} iconUrl={workspace.icon_url || ""} />
                                    <div>
                                        <p className="text-lg font-medium">{workspace.name}</p>
                                        <p className="text-sm text-muted-foreground">{workspace.member_count} {workspace.member_count === 1 ? 'Member' : 'Members'}</p>
                                    </div>
                                </div>
                                <div>
                                    {loadingWorkspaceId === workspace.id && <Loader2 className="text-center h-6 w-6 animate-spin opacity-50" />}
                                </div>
                            </div>
                        ))
                    )}
