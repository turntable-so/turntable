...
function BIProjectForm() {
  ...
  // 1. Define your form.
  const form = useForm<z.infer<typeof formBISchema>>({
    resolver: zodResolver(formBISchema),
    defaultValues: {
      dialect: "metabase",
      environment: "PROD",
    },
  ...
  async function onSubmit(values: z.infer<typeof formBISchema>) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <Card className="w-full rounded-sm">
          <CardHeader>
            <CardTitle className="text-xl">
              <div className="flex items-center">Select Git Repository</div>
            </CardTitle>
          </CardHeader>
          <CardContent>
  ...
  return (
    <div className="flex">
      <div className="w-[300px] flex flex-col h-screen py-12 bg-muted text-muted-foreground px-4 space-y-6">
        <div className="px-4 text-lg font-semibold">Add Source</div>
          ...
          {steps.map((tab: any, i: number) => (
            <div key={i}>
              <Button
                ...
                size="icon"
                className={`w-full rounded-lg ${currentStep === i ? "opacity-100" : "opacity-50"
                  } ${currentStep === i ? "text-black" : "bg-transparent"} `}
                aria-label={tab.title}
              >
                <div
                  className={`${currentStep === i ? "bg-[#ebebeb]" : "hover:bg-[#ebebeb]"
                    } px-4 p-2 rounded-lg w-full flex  space-x-2`}
                >
